from triton import TritonContext, ARCH, Instruction, OPCODE, EXCEPTION, CPUSIZE, MemoryAccess, MODE, AST_REPRESENTATION
...
# Create a symbolic area of memory for the output buffer
some_memory = MemoryAccess(output_buffer_location, 0x20)
ctx.symbolizeMemory(some_memory, "output_buffer")

# Place the address of the buffer on the stack
...
the_mem = ctx.getMemoryAst(some_memory)
print("\n\n[Expression used to generate output data]")
print("----------------------------------------\n")
print(ast.unroll(the_mem))
